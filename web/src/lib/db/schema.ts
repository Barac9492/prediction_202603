...
/** Time-series snapshots of computed probability for each thesis.
 *  Written after every feed ingest run or manual recompute.
 *  probability: 0.0–1.0 (maps to 0–100%)
 *  momentum: change vs previous snapshot (+/- float)
 */
export const thesisProbabilitySnapshots = pgTable(
  "thesis_probability_snapshots",
  {
    ...
    momentum: real("momentum"), // delta vs previous snapshot
    computedAt: timestamp("computed_at").notNull().defaultNow(),
    // optional: store top contributing news ids for drill-down
