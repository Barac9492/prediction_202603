...
export default async function GraphPage() {
  ...
  const thesisEdges: Record<number, { news: typeof newsNodes[0]; relation: string; reasoning: string | null }[]> = {};
    ...
    <div className="space-y-6">
                    ...
                    {connections.map((conn, i) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {conn.reasoning && (
                            <p className="text-xs text-zinc-500 mt-0.5 italic">{conn.reasoning}</p>
                          )}
                          <div className="flex items-center gap-2 mt-1">
                            {conn.news.source && (
                              <span className="text-xs text-zinc-600">{conn.news.source}</span>
                            )}
      ...
      {entityNodes.length > 0 && (
        <div className="rounded-lg border border-zinc-800 bg-zinc-900/30 p-4">
          <h2 className="text-sm font-semibold mb-3">Tracked Entities</h2>
          <div className="flex flex-wrap gap-2">
            {entityNodes.map((entity) => (
              ...
              >
                {entity.name}
                {entity.type !== "unknown" && (
                  <span className="text-zinc-500 ml-1">({entity.type})</span>
                )}
              </span>
            ))}
          </div>
