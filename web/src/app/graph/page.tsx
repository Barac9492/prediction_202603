...
export default async function GraphPage() {
  ...
  // For each thesis, find supporting/contradicting news via edges
              ...
              <div className="flex items-center justify-between">
                      <div>
                                ...
                                <p className="text-sm text-zinc-400 mt-1">
                                            How recent AI news connects to your investment theses.
                                </p>p>
                      </div>div>
                {entityNodes.length === 0 && newsNodes.length > 0 && (
                    <BackfillButton />
                  )}
              </div>div>
          ...
          {/* Stats */}
              <div className="flex gap-4 text-sm">
                      <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-3">
                                ...
                                <div className="text-xs text-zinc-500 mt-0.5">Active Theses</div>div>
                      </div>div>
                      <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-3">
                                <div className="text-2xl font-bold">{newsNodes.length}</div>div>
                                <div className="text-xs text-zinc-500 mt-0.5">News Events</div>div>
                      </div>div>
                      <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-3">
                                <div className="text-2xl font-bold text-purple-400">
                                  {entityNodes.length}
                                </div>div>
                                <div className="text-xs text-zinc-500 mt-0.5">Entities</div>div>
                      </div>div>
                      <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-3">
                                <div className="text-2xl font-bold text-orange-400">
                                  {totalConnections}
                                </div>div>
                                <div className="text-xs text-zinc-500 mt-0.5">Connections</div>div>
                      </div>div>
              </div>div>
        
          {thesisNodes.length === 0 ? (
                  <div className="rounded-lg border border-zinc-800 p-8 text-center text-zinc-500">
                            <p className="text-lg mb-2">No graph data yet</p>p>
                            <p className="text-sm">
                                        Start by creating theses at{" "}
                                        <a href="/thesis" className="text-blue-400 hover:underline">
                                                      /thesis
                                        </a>a>
                                        , then ingest news at{" "}
                  ...
                  <div className="space-y-4">
                    {thesisNodes.map((thesis) => {
                                ...
                                const supports = connections.filter(
                                                (c) => c.relation === "SUPPORTS"
                                                              ).length;
                                const contradicts = connections.filter(
                                                (c) => c.relation === "CONTRADICTS"
                                                              ).length;
                                                ...
                                                <div
                                                                ...
                                                                <div className="flex items-start justify-between mb-3">
                                                                                  <div>
                                                                                                      <div className="flex items-center gap-2 mb-1">
                                                                                                                            <span
                                                                                                                                                      className={`text-xs font-semibold px-2 py-0.5 rounded border ${
                                                                                                                                                                                  DIRECTION_COLORS[thesis.direction] ||
                                                                                                      ...
                                                                                                      <h2 className="font-semibold">{thesis.title}</h2>h2>
                                                                                    </div>div>
                                                                                  <div className="text-right shrink-0 ml-4">
                                                                                                      ...
                                                                                                      <div className="text-xs text-zinc-500">net score</div>div>
                                                  ...
                                                  {connections.length === 0 ? (
                                                                                    <p className="text-xs text-zinc-600 italic">
                                                                                                        No news connections yet. Process news events to generate
                                                                                                        links.
                                                                                      </p>p>
                                                                                  ) : (
                                                                                    <div className="space-y-2">
                                                                                      {connections.map((conn, i) => (
                                                                                                            <div
                                                                                                                                      key={i}
                                                                                                                                      className="flex items-start gap-3 rounded border border-zinc-800 bg-zinc-900/60 px-3 py-2"
                                                                                                                                    >
                                                                                                                                    <span
                                                                                                                                                                className={`text-xs font-semibold px-1.5 py-0.5 rounded border shrink-0 mt-0.5 ${
                                                                                                                                                                                              RELATION_STYLES[conn.relation] ||
                                                                                                                                                                                              RELATION_STYLES.MENTIONS
                                                                                                                                                                  }`}
                                                                                                                                                              >
                                                                                                                                      {conn.relation}
                                                                                                                                      </span>span>
                  ...
                  <div className="rounded-lg border border-zinc-800 bg-zinc-900/30 p-4">
                            <div className="flex items-center justify-between mb-3">
                                        <h2 className="text-sm font-semibold">
                                                      Tracked Entities ({entityNodes.length})
                                        </h2>h2>
                                        <BackfillButton />
                            </div>div>
