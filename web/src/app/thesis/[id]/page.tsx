...
export default async function ThesisDetailPage({
  ...
  const [history, thesisConnections, interactions, relatedEntities, allRecs] =
    await Promise.all([
      getProbabilityHistory(thesis.id, 90),
      getThesisConnections(thesis.id),
      getThesisInteractions(thesis.id),
      getEntitiesForThesis(thesis.id),
  ...
  const newsConnectionIds = thesisConnections
    ...
    .map((c) => c.fromId);
  const newsEvents = await getNewsEventsByIds(newsConnectionIds);
