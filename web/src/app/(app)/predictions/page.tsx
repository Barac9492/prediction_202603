...
async function getRecentSnapshots(workspaceId: string, limit = 30) {
    return db
      .select({
      ...
      .innerJoin(thesesTable, and(eq(thesisProbabilitySnapshots.thesisId, thesesTable.id), eq(thesesTable.workspaceId, workspaceId)))
...
export default async function LogPage({
  ...
  const [preds, stale, snapshots, allTheses] = await Promise.all([
        listPredictions(workspaceId, f),
        listStalePredictions(workspaceId, 7, 5),
        getRecentSnapshots(workspaceId, 50),
