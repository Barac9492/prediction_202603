          ...
          <div className="space-y-6">
                <div className="flex items-start justify-between">
                        ...
                        <div className="flex flex-col items-end gap-2">
                          ...
                          {processResult && (
                        <span className="text-xs text-zinc-400">{processResult}</span>span>
                                  )}
                        </div>div>
                </div>div>
            ...
            {/* Stats bar */}
                <div className="flex gap-4 text-sm">
                        ...
                        <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-3">
                                  ...
                                  <div className="text-xs text-zinc-500 mt-0.5">Processed (recent 20)</div>div>
                        </div>div>
                        <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-3">
                                  <div className="text-2xl font-bold text-green-400">
                                    {events.filter(e => e.sentiment === "bullish").length}
                                  </div>div>
                                  <div className="text-xs text-zinc-500 mt-0.5">Bullish signals</div>div>
                        </div>div>
                        <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-3">
                                  <div className="text-2xl font-bold text-red-400">
                                    {events.filter(e => e.sentiment === "bearish").length}
                                  </div>div>
                                  <div className="text-xs text-zinc-500 mt-0.5">Bearish signals</div>div>
                        </div>div>
                </div>div>
            ...
            {loading ? (
                    ...
                    <div className="rounded-lg border border-zinc-800 p-8 text-center text-zinc-500">
                              ...
                              <p className="text-sm">
                                          Run the Python ingester to populate news events:
                              </p>p>
                                                ...
                                                <div className="flex items-start justify-between gap-3">
                                                                <div className="flex-1 min-w-0">
                                                                                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                                                                                    {event.source && (
                                                                        <span className="text-xs text-zinc-500 shrink-0">{event.source}</span>span>
                                                                                                      )}
                                                                                    {event.sentiment && (
                                                                        <span
                                                                                                  className={`text-xs px-1.5 py-0.5 rounded border ${
                                                                                                                              SENTIMENT_STYLES[event.sentiment as keyof typeof SENTIMENT_STYLES] ||
                                                                        ...
                                                                        <span className="text-xs text-blue-400">
                                                                                                → {event.extractedThesisIds.length} thesis connection{event.extractedThesisIds.length > 1 ? "s" : ""}
                                                                        </span>span>
                                                                                                      )}
                                                                                    {!event.processed && (
