...
function RelevanceDots({ score }: { score: number | null }) {
    if (!score) return <span className="text-zinc-700">&mdash;</span>;
    return (
          ...
          <div className="space-y-6">
                <div className="flex items-start justify-between">
                        <div>
                                  ...
                                  <p className="text-sm text-zinc-400 mt-1">AI-relevant news from RSS feeds. Fetch → then Process to extract graph connections.</p>
                        </div>
                ...
                <div className="flex gap-4 text-sm">
                        ...
                        <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-3">
                                  <div className="text-2xl font-bold text-green-400">{events.filter((e) => e.sentiment === "bullish").length}</div>
                                  <div className="text-xs text-zinc-500 mt-0.5">Bullish</div>
                        </div>
                        <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 px-4 py-3">
                                  <div className="text-2xl font-bold text-red-400">{events.filter((e) => e.sentiment === "bearish").length}</div>
                                                ...
                                                <div className="flex items-start justify-between gap-3">
                                                                <div className="flex-1 min-w-0">
                                                                                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                                                                                    {event.source && <span className="text-xs text-zinc-500 shrink-0">{event.source}</span>}
                                                                                    {event.sentiment && (
                                                        ...
                                                        <div className="flex flex-wrap gap-1 mt-1.5">
                                                          {event.extractedEntities.slice(0, 6).map((entity) => (
                                                                                  <span key={entity} className="text-xs text-zinc-500 bg-zinc-800 px-1.5 py-0.5 rounded">{entity}</span>
                                                                                ))}
                                                        </div>
                                                                                  )}
                                                                </div>
