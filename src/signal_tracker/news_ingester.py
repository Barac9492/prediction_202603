...
AI_RSS_FEEDS = [
    ...
    {"url": "https://techcrunch.com/category/artificial-intelligence/feed/", "source": "TechCrunch AI"},
      ...
      {"url": "https://hnrss.org/frontpage?q=AI+OR+LLM+OR+GPT+OR+model&count=20", "source": "HN"},
      {"url": "https://hnrss.org/frontpage?q=NVIDIA+OR+OpenAI+OR+Anthropic+OR+investment&count=10", "source": "HN"},
]
...
def fetch_rss(feed_url: str, source: str, timeout: int = 10) -> list[NewsItem]:
    ...
    try:
          ...
          print(f"  [WARN] Failed to parse XML from {feed_url}: {e}")
          return []
    ...
    if is_atom:
            ...
            if title and url:
                              items.append(NewsItem(title=title, url=url, source=source,
...
def ingest_all_feeds(db_url: Optional[str] = None) -> dict:
    ...
    print(f"Fetching {len(AI_RSS_FEEDS)} RSS feeds...")
    ...
    print(f"Found {len(all_items)} AI-relevant items after dedup.")
...
def _write_to_db(items: list[NewsItem], db_url: str) -> dict:
    ...
    for item in items:
            ...
            print(f"  [WARN] Failed to insert {item.url}: {e}")
            conn.rollback()
            continue
    ...
    print(f"Inserted {inserted} new items, skipped {skipped} duplicates.")
