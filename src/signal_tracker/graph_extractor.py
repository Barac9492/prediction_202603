...
def process_batch(db_url: str, limit: int = 20, dry_run: bool = False) -> dict:
    ...
    print(f"Processing {len(events)} events against {len(theses)} active theses...")
    ...
    for event in events:
        ...
        try:
                      ...
                      print(f"    AI Relevance: {result.ai_relevance}/5 | Sentiment: {result.sentiment}")
            ...
            if not dry_run:
                ...
                for tc in result.thesis_connections:
                    ...
                    total_connections += 1
                    print(f"    -> {tc['relation']} Thesis #{tc['thesis_id']} "
                                                    f"(confidence: {tc.get('confidence', 0.5):.2f})")
